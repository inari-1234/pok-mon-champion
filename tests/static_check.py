...
assert len([x for x in soup.find_all('script',src=True) if x.get('src','').split('?')[0]=='core.js'])==1, 'core.js loaded more than once'
...
assert 'bottom-nav' not in html, 'legacy bottom nav must be removed from production HTML'
