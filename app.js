(() => {
  ...
  function defaultState() {
    return { version: 9, rank: '', team: { name: '', format: 'single', members: [], favorite: '', plan: '', weak: '' }, inventory: { unowned: [] }, matches: [], metaNotes: [], onboarding: { trainingComplete: false } };
  }
  ...
  const FORM_SPRITE_IDS = {
    raichualola:10100,ninetalesalola:10104,persianalola:10108,arcaninehisui:10230,slowbrogalar:10165,
    taurospaldeacombat:10250,taurospaldeablaze:10251,taurospaldeaaqua:10252,typhlosionhisui:10233,
    slowkinggalar:10172,rotomheat:10008,rotomwash:10009,rotomfrost:10010,rotomfan:10011,rotommow:10012,
    samurotthisui:10236,zoroarkhisui:10239,stunfiskgalar:10180,goodrahisui:10242,gourgeistsmall:10030,
    gourgeistlarge:10031,gourgeistsuper:10032,avalugghisui:10243,decidueyehisui:10244,lycanrocmidnight:10126,
    lycanrocdusk:10152,indeedeef:10186,basculegionf:10248
  };
  function spriteUrl(mon) {
    if(!mon || mon.dex<=0) return '';
  ...
  function markUnowned(monOrValue) {
    ...
    state.onboarding ||= {trainingComplete:false}; state.onboarding.trainingComplete=false;
    ...
    const advice=el('teamStarterAdvice'); if(advice){advice.hidden=false;advice.textContent=`${mon.name}を外しました。代わりの候補を表示します。`;}
  }
  ...
  function renderTeamDraft() {
    ...
    el('autoCompleteTeam').hidden=teamDraft.length>=6;
  ...
  function commitPokemonPicker() {
    ...
    if(pickerContext.kind==='starter'){
      favoriteDraft=picked[0]||'';
      const built=C.completeStarterTeam(picked,el('teamFormat').value||state.team.format,catalog,[...unownedIds()]);
      teamDraft=built.members.length?built.members:picked;
      renderTeamDraft();
      const advice=el('teamStarterAdvice');
  ...
  function sanitizeImported(data) {
    ...
    clean.onboarding = {
      trainingComplete: typeof onboarding.trainingComplete === 'boolean'
        ? onboarding.trainingComplete
        : clean.matches.some(m=>m.format===clean.team.format)
    };
  ...
  function renderDashboard() {
    ...
    const advanced=el('homeAdvancedMenu'); if(advanced) advanced.hidden=count<6;
  ...
  function renderTeamBuilds() {
    ...
    const isLast=trainingViewIndex===sets.length-1;
    const next=make('button','primary-btn',isLast?'育成を確認して対戦へ':'次のポケモン');next.type='button';next.addEventListener('click',()=>{
      if(isLast){
        state.onboarding ||= {trainingComplete:false}; state.onboarding.trainingComplete=true; saveState(); renderDashboard(); navigate('assist'); return;
      }
      trainingViewIndex+=1;renderTeamBuilds();el('teamTrainingPanel').scrollIntoView({behavior:'smooth',block:'start'});
    });
